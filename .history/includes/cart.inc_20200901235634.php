...
*/
    ...
    if ($products_in_cart) {
        // there are products in the cart, we need to select those products from the DBh
        // products in the cart array to question mark string array, we need the SQL statement to inculed in (?,?,?,...)
        $array_to_question_marks = implode(',', array_fill(0, count($products_in_cart), '?'));

        $stmt = $pdo->prepare('SELECT * FROM product p WHERE p.id IN ('. $array_to_question_marks .')');
